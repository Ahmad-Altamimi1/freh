...
description: |
  Guide for building features, pages, tables, forms, themes, and navigation in this Next.js 16 shadcn dashboard template. Use this skill whenever the user wants to add a new page, create a feature module, build a data table, add a form, configure navigation items, add a theme, set up RBAC access control, or work with the dashboard's patterns and conventions. Also triggers when adding routes under /dashboard, working with Supabase auth and RBAC, creating mock APIs, or modifying the sidebar. Even if the user doesn't mention "dashboard" explicitly — if they're adding UI, pages, or features to this project, use this skill.
---
...
Configure in `src/config/nav-config.ts`. Items are filtered client-side in `src/hooks/use-nav.ts` from the server-seeded `SessionProvider` context.
...
Both are read from the Supabase user's `app_metadata` (see `src/lib/auth/roles.ts`). Items without `access` are visible to everyone. All checks are synchronous — no loading states.

Nav filtering is UX only. Guard the page itself with `requireUser()` from `@/lib/auth/session` plus `hasAnyRole()`/`hasPermission()` from `@/lib/auth/roles`. See `docs/nav-rbac.md`.
