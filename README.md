...
Auth, database, and private file storage are wired to Supabase and function end-to-end.
...
- Database - [Supabase Postgres](https://supabase.com/docs/guides/database) + [Drizzle ORM](https://orm.drizzle.team)
- File storage - [Supabase Storage](https://supabase.com/docs/guides/storage) (private bucket)
...
- Authentication with Supabase Auth: email/password sign-in, server-verified sessions, protected routes
- PostgreSQL via Drizzle ORM with migration tooling — schema intentionally empty, ready for your model
- Private file storage with validation, unguessable paths, and short-lived signed URLs
- Generic audit logging abstraction with pluggable sinks
- Client-side RBAC navigation that filters menu items by role and permission
...
| Page                                                                                                                                                                  | Notes                                                                                                                                                                                |
| :------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Sign In | Email/password sign-in via Supabase Auth. Unauthenticated traffic to `/dashboard` is redirected here and returned to its original destination afterwards. |
...
See `env.example.txt` for the variables you need. They cover Supabase (auth, database, storage) and error tracking.

##### Supabase setup

Create a project, add users, create a private storage bucket, and run migrations — see [supabase-setup.md](./docs/supabase-setup.md).

There is no sign-up page: users are created in the Supabase dashboard. The database schema starts empty by design.
...
Yes. Auth is isolated behind `src/lib/auth/session.ts`, `src/lib/supabase/`, `src/proxy.ts`, and `src/features/auth/`. Swap those and the rest of the app is unaffected.

**Where do I define my database tables?**
In `src/db/schema/`, which ships empty on purpose. Add your tables, then run `bun run db:generate` and `bun run db:migrate`. Remember to enable Row Level Security on anything the anon key can reach.
...
docker build \
  ...
  --build-arg NEXT_PUBLIC_SUPABASE_ANON_KEY=eyJxxxxx \
  -t shadcn-dashboard .
...
docker run -d -p 3000:3000 \
  ...
  -e NEXT_PUBLIC_SUPABASE_ANON_KEY=eyJxxxxx \
  -e SUPABASE_SERVICE_ROLE_KEY=eyJxxxxx \
  -e SUPABASE_STORAGE_BUCKET=private \
